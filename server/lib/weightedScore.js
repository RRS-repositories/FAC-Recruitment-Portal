/**
 * The marking rule for the assessments that carry negative weights -- the
 * Sales & Customer Service and AI Developer roles -- in one place.
 *
 ...
 * These questions do have them, and the designs they were written with score
 * them this way:
 *
 ...
 *   - the total is a percentage of that maximum, rounded, and clamped to
 *     0..100.
 *
 * Applying the paralegal rule to these would penalise a candidate twice for
 * the same wrong tick. Scored on the server, from weights the browser never
 * received -- the client's own number, if it shows one, is never used.
 *
 * Plain functions of their arguments, no imports: each role's own scoring
 * module fixes the question list and the maximum.
 */
...
export function weightedScore(answers, questions, max = maxScoreFor(questions)) {
  if (max <= 0) return 0;
